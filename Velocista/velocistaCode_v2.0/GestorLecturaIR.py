...
#tiempo en s que equivale al 100% de negro
TIEMPO_MAX=0.0035
...
class GestorLecturaIR():
    ...
    #Cada vez que se ejcuta comprueba si ha habido un flanco de bajada y por lo tanto se puede tomar una medida
        for pin in range(len(pin_control_sensor)):
            ...
            if self.estado_anterior[pin]==1 and self.estado_anterior[pin]!=GPIO.input(pin_control_sensor[pin]):
                tiempo_transcurrido=time() - self.tiempo_inicio_medida #en segundos
                #normalizar tiempo entre 0 y 100
                tiempo_normalizado=(tiempo_transcurrido/TIEMPO_MAX)*100
                self.sensor_out[pin]=tiempo_normalizado
                
            #actualizar valores de estado_anterior
            self.estado_anterior[pin]=GPIO.input(pin_control_sensor[pin])
            print(self.estado_anterior)
